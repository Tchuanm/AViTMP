...
import torch
from pytracking.evaluation.box_ops import box_iou
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            # print('IOU before previous frame bbox with current bbox:', IOU, frame_num)
            # if frame_num >= 100:
            if out['flag'] != 3 or out['object_presence_score'] < 0.5 or IOU <= 0.1: 
                # print('IOU. flag, score:', IOU, out['flag'], out['object_presence_score'])
                self.backtrack_times += 1
                # running CycleTrack
                info_gt['previous_output'] = prev_output
                out1 = tracker.track(self.prev_img, info_gt, back_gt=True)
                IOU1, _ = box_iou(out1['target_bbox'], prev_output['target_bbox'])
                # print('IOU1. flag1, score1:', IOU1, out1['flag'], out1['object_presence_score'])
                if out['output_state_2nd'] is not None and IOU1 <= 0.1 and out1['flag'] != 3 and out1['object_presence_score'] < 0.5:
                # bbox1 backtrack fail gt, backtrack 2nd bbox
                    pre_out_new = copy.copy(out)
                    pre_out_new['target_bbox'] = out['output_state_2nd']
                    pre_out_new['output_state_2nd'] = None
                    out2 = tracker.track2(self.prev_img, pre_out_new, back_gt=False, back_2=True)
                    IOU2, _ = box_iou(out2['target_bbox'], prev_output['target_bbox'])
                    # print('IOU2. flag2, score2:', IOU2, out2['flag'], out2['object_presence_score'])
                    if IOU2 >= 0.1 and out2['object_presence_score'] >= 0.3 and out2['flag'] == 3: # only change with normal flag 
                        print('Choose subpeak!!!!!!!!!!') 
                        out['target_bbox'] = out['output_state_2nd']
                        self.subpeak_choose += 1
        ...
        print('backtrack, subpeak, not choose subpeak time ', self.backtrack_times, self.subpeak_choose, self.not_subpeak_choose)
