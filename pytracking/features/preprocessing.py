...
def numpy_to_torch(a: np.ndarray):
    ...
    # return torch.from_numpy(a).float().unsqueeze(0).unsqueeze(0).repeat(1,3,1,1)
...
def sample_patch(im: torch.Tensor, pos: torch.Tensor, sample_sz: torch.Tensor, output_sz: torch.Tensor = None,
    ...
    if mode == 'inside' or mode == 'inside_major':
        ...
        outside = torch.div(((-tl).clamp(0) + (br - im2_sz).clamp(0)), 2, rounding_mode='trunc')
    ...
    if not is_mask:
        ...
        im_patch = F.interpolate(im_patch, output_sz.long().tolist(), mode='nearest', align_corners=False)
