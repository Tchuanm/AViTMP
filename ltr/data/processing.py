...
class LTRBDenseRegressionProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        # data['train_sample_region'][-1] = ~ data['train_sample_region'][-1] 
        # x,y,w,h = data['train_anno'][-1]           # mask train2 image by mean pixel
        # data['train_images'][-1][:,int(x):int(x+w), int(y):int(y+h)] = data['train_images'][-1].mean()
        # # Guassian ltrb and center labels  0-1 convert each other. 
        # data['train_ltrb_target'][-1] = data['train_ltrb_target'][-1].mean() 
        # data['train_label'][-1]  = data['train_label'][-1].mean()
