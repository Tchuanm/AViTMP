...
import torch
from ltr.actors.heatmaps import generate_heatmap
...
class AViTMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        # Classification losses for the different optimization iterations
        clf_loss_hinge = self.objective['test_clf'](target_scores, data['test_label'], data['test_anno'])

        # gt_bbox = data['test_anno'][-1]  # (Ns, batch, 4) (x1,y1,w,h) -> (batch, 4)
        # gt_gaussian_maps = generate_heatmap(data['test_anno'], patch_size=288, stride=16)
        # gt_gaussian_maps = gt_gaussian_maps[-1].unsqueeze(0)
        # clf_loss_focal = self.objective['focal_cls'](target_scores, gt_gaussian_maps)

        loss = self.loss_weight['giou'] * loss_giou + self.loss_weight['test_clf'] * clf_loss_hinge # + self.loss_weight['test_focal'] * clf_loss_focal
        ...
        stats = {'Loss/total': loss.item(),
                 ...
                 'Loss/clf_loss_hinge': clf_loss_hinge.item(),
                 'Loss/weighted_clf_loss_hinge': self.loss_weight['test_clf']*clf_loss_hinge.item(),
                #  'Loss/clf_loss_focal': clf_loss_focal.item(),
                #  'Loss/weighted_clf_loss_bce': self.loss_weight['test_focal']*clf_loss_focal.item(),
                 'mIoU': ious.mean().item(),
